...
export class InfoLaboralService {
   ...
   url = environment.api+ 'infolaboral/';
   ...
   post(infolaboral: InfoLaboral): Promise<any> {
      return this.http.post(this.url, JSON.stringify(infolaboral), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(infolaboral: InfoLaboral): Promise<any> {
      return this.http.put(this.url, JSON.stringify(infolaboral), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
