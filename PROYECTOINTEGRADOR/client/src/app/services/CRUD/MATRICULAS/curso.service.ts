...
export class CursoService {
   ...
   post(curso: Curso): Promise<any> {
      return this.http.post(this.url, JSON.stringify(curso), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(curso: Curso): Promise<any> {
      return this.http.put(this.url, JSON.stringify(curso), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
