...
export class TipoPersonaService {
   ...
   url = environment.api + 'tipopersona/';
   ...
   post(tipopersona: TipoPersona): Promise<any> {
      return this.http.post(this.url, JSON.stringify(tipopersona), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(tipopersona: TipoPersona): Promise<any> {
      return this.http.put(this.url, JSON.stringify(tipopersona), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
