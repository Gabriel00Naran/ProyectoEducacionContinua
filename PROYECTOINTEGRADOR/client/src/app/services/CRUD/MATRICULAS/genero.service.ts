...
export class GeneroService {
   ...
   post(genero: Genero): Promise<any> {
      return this.http.post(this.url, JSON.stringify(genero), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(genero: Genero): Promise<any> {
      return this.http.put(this.url, JSON.stringify(genero), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
