...
export class NivelService {
   ...
   post(nivel: Nivel): Promise<any> {
      return this.http.post(this.url, JSON.stringify(nivel), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(nivel: Nivel): Promise<any> {
      return this.http.put(this.url, JSON.stringify(nivel), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
