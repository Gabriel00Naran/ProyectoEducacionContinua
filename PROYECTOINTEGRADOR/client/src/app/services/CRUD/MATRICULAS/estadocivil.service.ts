...
export class EstadoCivilService {
   ...
   url = environment.api + 'estadocivil/';
   ...
   post(estadocivil: EstadoCivil): Promise<any> {
      return this.http.post(this.url, JSON.stringify(estadocivil), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(estadocivil: EstadoCivil): Promise<any> {
      return this.http.put(this.url, JSON.stringify(estadocivil), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
