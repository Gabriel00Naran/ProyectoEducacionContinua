...
export class MatriculaService {
   ...
   post(matricula: Matricula): Promise<any> {
      return this.http.post(this.url, JSON.stringify(matricula), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(matricula: Matricula): Promise<any> {
      return this.http.put(this.url, JSON.stringify(matricula), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
