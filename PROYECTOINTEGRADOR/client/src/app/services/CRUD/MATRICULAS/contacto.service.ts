...
export class ContactoService {
   ...
   post(contacto: Contacto): Promise<any> {
      return this.http.post(this.url, JSON.stringify(contacto), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(contacto: Contacto): Promise<any> {
      return this.http.put(this.url, JSON.stringify(contacto), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
