...
export class ModalidadService {
   ...
   post(modalidad: Modalidad): Promise<any> {
      return this.http.post(this.url, JSON.stringify(modalidad), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(modalidad: Modalidad): Promise<any> {
      return this.http.put(this.url, JSON.stringify(modalidad), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
